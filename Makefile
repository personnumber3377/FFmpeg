...
# tools/target_graph_fuzzer_coverage.c

tools/target_graph_fuzzer_coverage$(EXESUF): tools/target_graph_fuzzer_coverage.o $(FF_DEP_LIBS)
	clang $(LDFLAGS) -fsanitize=address,undefined,fuzzer-no-link $(LDEXEFLAGS) $(LD_O) $^ $(ELIBS) $(FF_EXTRALIBS)

# tools/betterfuzzer.c


tools/betterfuzzer$(EXESUF): tools/betterfuzzer.o $(FF_DEP_LIBS)
	clang $(LDFLAGS) -fsanitize=address,undefined,fuzzer-no-link $(LDEXEFLAGS) $(LD_O) $^ $(ELIBS) $(FF_EXTRALIBS)
